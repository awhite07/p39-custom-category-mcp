import { z } from 'zod';
// Language codes from Peer39 PDF — exact set. "All" is the wildcard for category APIs;
// "all" (lowercase) is the wildcard for the get-url-examples endpoint per its PDF example.
...
// For get-url-examples the wildcard is lowercase "all".
export const URL_EXAMPLES_LANGUAGES = ['all', ...LANGUAGE_CODES.filter((c) => c !== 'All')];
...
// expirationDate: YYYY-MM-DD, between today and ~1 year from today.
// Tolerance of ±1 day so callers aren't tripped up by timezone math.
...
// for type first, then DSP, then items, then a name. The rest auto-default.
// partnerId is REQUIRED — testers always pick the DSP per category; there is no
// implicit fallback to a saved default.
...
}).refine((v) => !v.itemsTypes || v.itemsTypes.length === v.items.length, { message: 'itemsTypes length must equal items length when provided', path: ['itemsTypes'] }).refine((v) => !v.itemsTypes || v.type === 2, { message: 'itemsTypes is only valid for keyword categories (type=2)', path: ['itemsTypes'] });
...
export const UpdateCategoryItemsInputSchema = z.object({
    ...
    items: ItemsArraySchema,
    itemsTypes: z.array(ItemsTypeSchema).optional(),
    // Note: Peer39 API defaults this to FALSE (replace). We invert at the tool layer for safety.
    append: z.boolean().optional(),
}).refine((v) => !v.itemsTypes || v.itemsTypes.length === v.items.length, { message: 'itemsTypes length must equal items length when provided', path: ['itemsTypes'] });
export const UpdateCategoryInputSchema = z.object({
    ...
    description: z.string().optional(),
    advertiserId: z.string().optional(),
}).refine((v) => !v.itemsTypes || !v.items || v.itemsTypes.length === v.items.length, { message: 'itemsTypes length must equal items length when both are provided', path: ['itemsTypes'] });
...
export const ConfigureInputSchema = z.object({
    ...
    defaultPartnerId: PartnerIdInputSchema.optional(),
}).refine((v) => Object.values(v).some((x) => x !== undefined), { message: 'Provide at least one field to set.' });
