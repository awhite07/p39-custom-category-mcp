...
export const PARTNER_NAME_TO_ID = Object.freeze({
    'mediamath': 730,
...
// Reverse lookup: numeric ID -> canonical slug. Returns the FIRST name that
// maps to the given ID (so aliases like "xandr" lose to "microsoft-advertising"
// because the canonical name is listed first in PARTNER_NAME_TO_ID).
export function partnerIdToName(id) {
...
export function resolvePartnerId(input) {
    ...
    if (id === undefined) {
        throw new Error(`Unknown partner name "${input}". Known names: ${Object.keys(PARTNER_NAME_TO_ID).join(', ')}. ` +
