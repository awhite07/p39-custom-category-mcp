...
export const configureTool: ToolDefinition = {
...
- defaultPartnerId: numeric DSP id OR a friendly name like "the-trade-desk", "xandr", "basis"`,
  ...
  async handler(rawArgs) {
    ...
    try {
      ...
      if (parsed.data.buyerId !== undefined) patch.buyerId = parsed.data.buyerId;
      if (parsed.data.system !== undefined) patch.system = parsed.data.system;
      if (parsed.data.userEmail !== undefined) patch.userEmail = parsed.data.userEmail;
      if (parsed.data.defaultPartnerId !== undefined) {
        patch.defaultPartnerId = resolvePartnerId(parsed.data.defaultPartnerId);
      }
      const merged = await updateRuntimeConfig(patch as never);
      return {
