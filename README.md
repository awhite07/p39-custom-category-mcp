...
```bash
curl -fsSL https://raw.githubusercontent.com/awhite07/p39-custom-category-mcp/v1.0.9/scripts/preflight.sh -o /tmp/peer39-integration.sh
bash /tmp/peer39-integration.sh
```

> The URL pins to a specific release tag (`v1.0.9`) rather than `main` so you always get a known-good script — the GitHub CDN can lag behind `main` by several minutes after a push.
...
```json
{
  "mcpServers": {
    "peer39": {
      "command": "npx",
      "args": ["-y", "github:awhite07/p39-custom-category-mcp#v1.0.9"],
      "env": {
...
Testers running the curl URL or `npx -y github:awhite07/p39-custom-category-mcp#v1.0.9` will resolve to the new commit immediately — no CDN cache lag because each tag is an immutable ref.
